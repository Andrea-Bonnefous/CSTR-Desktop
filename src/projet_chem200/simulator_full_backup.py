...
class CSTRSimulatorApp:
    """GUI application for CSTR Simulator with reaction database"""
    
    def __init__(self, master):
        self.master = master
        self.master.title("CSTR Simulator")
        self.master.geometry("1200x800")
        
        # Initialize simulator and database
        ...
        # Initialize with first reaction in the database
        self.reaction_selector.current(0)
        self._update_reaction_details()
    ...
    def _create_widgets(self):
        ...
        # Reaction selection
        self.reaction_label = ttk.Label(self.left_frame, text="Select Reaction:", font=("Arial", 12, "bold"))
        self.reaction_selector = ttk.Combobox(self.left_frame, width=40, state="readonly")
        self.reaction_selector["values"] = self.database.get_reaction_names()
        self.reaction_selector.bind("<<ComboboxSelected>>", lambda e: self._update_reaction_details())
        
        # Reaction details
        self.details_frame = ttk.LabelFrame(self.left_frame, text="Reaction Details", padding=10)
        self.details_text = tk.Text(self.details_frame, wrap=tk.WORD, width=50, height=15)
        self.details_text.config(state=tk.DISABLED)
        
        # Reactor parameters
        self.params_frame = ttk.LabelFrame(self.left_frame, text="Reactor Parameters", padding=10)
        
        self.volume_label = ttk.Label(self.params_frame, text="Reactor Volume (m³):")
        self.volume_entry = ttk.Entry(self.params_frame, width=10)
        self.volume_entry.insert(0, "5.0")
        
        self.flow_label = ttk.Label(self.params_frame, text="Feed Flow Rate (m³/s):")
        self.flow_entry = ttk.Entry(self.params_frame, width=10)
        self.flow_entry.insert(0, "0.02")
        
        self.recycle_label = ttk.Label(self.params_frame, text="Recycle Ratio (0-1):")
        self.recycle_entry = ttk.Entry(self.params_frame, width=10)
        self.recycle_entry.insert(0, "0.2")
        
        self.temp_label = ttk.Label(self.params_frame, text="Initial Temperature (K):")
        self.temp_entry = ttk.Entry(self.params_frame, width=10)
        self.temp_entry.insert(0, "500")
        
        self.optimize_var = tk.BooleanVar(value=True)
        self.optimize_check = ttk.Checkbutton(
            self.params_frame, 
            text="Optimize Temperature",
        ...
        # Results area
        self.results_frame = ttk.LabelFrame(self.right_frame, text="Simulation Results", padding=10)
        self.results_text = tk.Text(self.results_frame, wrap=tk.WORD, width=60, height=30)
    ...
    def _layout_widgets(self):
        ...
        # Left frame layout
        self.reaction_label.grid(row=0, column=0, sticky="w", pady=(0, 5))
        self.reaction_selector.grid(row=1, column=0, sticky="ew", pady=(0, 10))
        
        self.details_frame.grid(row=2, column=0, sticky="ew", pady=(0, 10))
        self.details_text.pack(fill=tk.BOTH, expand=True)
        
        self.params_frame.grid(row=3, column=0, sticky="ew", pady=(0, 10))
        
        # Parameters layout
        self.volume_label.grid(row=0, column=0, sticky="w", pady=2)
        self.volume_entry.grid(row=0, column=1, sticky="e", pady=2)
        
        self.flow_label.grid(row=1, column=0, sticky="w", pady=2)
        self.flow_entry.grid(row=1, column=1, sticky="e", pady=2)
        
        self.recycle_label.grid(row=2, column=0, sticky="w", pady=2)
        self.recycle_entry.grid(row=2, column=1, sticky="e", pady=2)
        
        self.temp_label.grid(row=3, column=0, sticky="w", pady=2)
        self.temp_entry.grid(row=3, column=1, sticky="e", pady=2)
        
        self.optimize_check.grid(row=4, column=0, columnspan=2, sticky="w", pady=5)
        
        self.run_button.grid(row=4, column=0, sticky="ew", pady=10)
        
        # Right frame layout
        self.results_frame.grid(row=0, column=0, sticky="nsew", pady=(0, 10))
    ...
    def _update_reaction_details(self):
        """Update the reaction details when a new reaction is selected"""
        selected_reaction = self.reaction_selector.get()
        reaction_data = self.database.get_reaction_details(selected_reaction)
        ...
        self.details_text.insert(tk.END, f"Description: {reaction_data['description']}\n\n")
        self.details_text.insert(tk.END, f"Target Product: {reaction_data['target_product']}\n")
        self.details_text.insert(tk.END, f"Catalyst: {reaction_data['catalyst']}\n")
        self.details_text.insert(tk.END, f"Temperature Range: {reaction_data['temperature_range'][0]}-{reaction_data['temperature_range'][1]} K\n\n")
        
        # Display individual reactions
        self.details_text.insert(tk.END, "Reactions:\n")
        for i, reaction in enumerate(reaction_data['reactions']):
            self.details_text.insert(tk.END, f"{i+1}. {reaction['name']}\n")
            ...
            self.details_text.insert(tk.END, f"   {stoich_str}\n")
            
            # Is reversible?
            if reaction.get('reversible', False):
                self.details_text.insert(tk.END, "   (Reversible reaction)\n")
            
        # Display feed composition
        self.details_text.insert(tk.END, "\nFeed Composition:\n")
        for comp, conc in reaction_data['feed_composition'].items():
            self.details_text.insert(tk.END, f"   {comp}: {conc} mol/m³\n")
        
        self.details_text.config(state=tk.DISABLED)
        
        # Update temperature entry with middle of range
        temp_range = reaction_data['temperature_range']
        mid_temp = (temp_range[0] + temp_range[1]) / 2
        self.temp_entry.delete(0, tk.END)
        self.temp_entry.insert(0, f"{mid_temp:.1f}")
    
    def _run_simulation(self):
        """Run the simulation with the selected reaction and parameters"""
        try:
            # Get reaction data
            selected_reaction = self.reaction_selector.get()
            reaction_data = self.database.get_reaction_details(selected_reaction)
            ...
            if not reaction_data:
                messagebox.showerror("Error", "No reaction selected!")
                return
            
            # Get reactor parameters
            volume = float(self.volume_entry.get())
            flow_rate = float(self.flow_entry.get())
            recycle_ratio = float(self.recycle_entry.get())
            temperature = float(self.temp_entry.get())
            optimize_temp = self.optimize_var.get()
            
            # Validate parameters
            if volume <= 0 or flow_rate <= 0 or recycle_ratio < 0 or recycle_ratio >= 1:
                messagebox.showerror("Error", "Invalid reactor parameters!")
                return
            ...
            # Run simulation
            temp_bounds = reaction_data['temperature_range']
            results = self.simulator.run_simulation(optimize_temp=optimize_temp, temp_bounds=temp_bounds)
            
            # Display results in text area
            self._display_results(results)
            
            # Create and display visualizations
            self._display_visualization(results)
    ...
    def _display_results(self, results):
        ...
        # Format results
        self.results_text.insert(tk.END, "===== CSTR SIMULATION RESULTS =====\n\n")
        self.results_text.insert(tk.END, f"Optimal Temperature: {results['temperature']:.2f} K\n")
        self.results_text.insert(tk.END, f"Product Yield: {results['yield']*100:.2f}%\n")
        
        if results.get('catalyst'):
            self.results_text.insert(tk.END, f"Catalyst: {results['catalyst']}\n")
        
        self.results_text.insert(tk.END, "\nSteady State Concentrations (mol/m³):\n")
        for component, conc in results["concentrations"].items():
            if component == self.simulator.target_product:
                self.results_text.insert(tk.END, f"  {component}: {conc:.4f} (TARGET PRODUCT)\n")
            else:
                self.results_text.insert(tk.END, f"  {component}: {conc:.4f}\n")
        
        self.results_text.insert(tk.END, "\nRecycle Stream Concentrations (mol/m³):\n")
        for component, conc in results["recycle_stream"].items():
            self.results_text.insert(tk.END, f"  {component}: {conc:.4f}\n")
        
        self.results_text.insert(tk.END, f"\nResidence Time: {results['residence_time']:.2f} s\n")
        
        self.results_text.insert(tk.END, "\nReaction Rates (mol/m³·s):\n")
        for name, rate in results["reaction_rates"].items():
            self.results_text.insert(tk.END, f"  {name}: {rate:.6f}\n")
        
        self.results_text.insert(tk.END, "\nReactant Conversions:\n")
        for component, conversion in results["conversions"].items():
            if component in self.simulator.feed_composition:
                self.results_text.insert(tk.END, f"  {component}: {conversion*100:.2f}%\n")
    ...
    def _display_visualization(self, results):
        """Create and display visualizations"""
        ...
        fig, axes = self.simulator.create_visualization(results)
